...
#%%
expt_dir = "./weights/expt_240325_223505_797589"
...
MATRIC_LIST = ['ndcg', 'recall', 'precision']
...
with open(train_data_path, 'r') as inp:
    ...
    positive_pairs: list = list(filter(lambda pair: pair[2] > 0, pairs)) # feedback 이 0 이상인거
    user_positive_interaction: list = analyse_user_interacted_set(positive_pairs) # user_id 별로 feedback이 0 이상인 것들
    inp.close()
...
with open(test_data_path, 'r') as inp:
    ...
    ground_truth: list = analyse_user_interacted_set(pairs) # user_id별로 feedback이 0이상인 것들
    inp.close()
...
#%%
checkpoint_dir = f"./weights/expt_240325_223505_797589"
epoch_num = 100
...
for (batch_index, 
     (batch_users_tensor, 
      batch_items_tensor, 
      batch_scores_tensor, 
      batch_envs_tensor)) in tqdm(enumerate(mini_batch(batch_size, 
                                                       users_tensor, 
                                                       items_tensor, 
                                                       scores_tensor, 
                                                       envs))):
...
inv_prefs = torch.concat(inv_prefs, dim = 0).numpy()
var_prefs = torch.concat(var_prefs, dim = 0).numpy()
total_loss = torch.concat(total_loss, dim = 0).numpy()
...
plt.plot(range(0, 110, 50), env_losses)
plt.xlabel("Epochs")
plt.ylabel("NLL loss")
...
np.random.seed(seed)
indices = np.random.choice(inv_pref.shape[0], sample_num) # inv_prefs?
